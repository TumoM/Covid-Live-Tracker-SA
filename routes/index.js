...
router.get("/", async function (req, res) {
    ...
    const result = await pool.connect(async (connection) => {
        ...
        return 'foo';
  ***REMOVED***);
    // {data:value,provCases,provDeaths,provRecoveries,graphData***REMOVED***
...
let getInfoSol = async function(pool) {
    return pool.connect(async (connection) => {
        let value = await connection.query(sql` select "provinceName", "provDate", "caseCount", "deathCount", "recovered" from "provinceDays" order by "provDate" desc limit ${1***REMOVED***`);
        if (res1[0].totalTests === null){
            return knex('dates')
                .select('date','totalTests')
                .whereNotNull('totalTests')
      ...
      ***REMOVED***
        ...
        let value1 = await connection.query(sql` select "provinceName", "provDate", "caseCount", "deathCount", "recovered" from "provinceDays" order by "provDate" desc limit ${1***REMOVED***`);
        let graphData = await connection.query(sql` select "provinceName", "provDate", "caseCount", "deathCount", "recovered" from "provinceDays" order by "provDate" desc limit ${1***REMOVED***`);
  ***REMOVED***);
    return knex.transaction((trx) =>{
        return knex('dates')
